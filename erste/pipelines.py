...
class ErstePipeline:
    ...
    # Database setup
    conn = sqlite3.connect('erste.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `erste`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM erste WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO erste (date, title, link, content)"
